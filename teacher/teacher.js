...
async function fetchTerm1Grades() {
  ...
  try {
    ...
    if (selectedPlatform === 'A_Level') {
      levelPromise = supabase
        .from(lTable)
        .select('studentid, reached_level')
        .in('studentid', usernames);
    } else {
      levelPromise = supabase
        .from(lTable)
        .select('username, level_number, level_done')
        .in('username', usernames);
    }

    const [{ data: theoryData, error: theoryErr }, { data: rawLevelData, error: levelErr }] = await Promise.all([
      theoryPromise,
      levelPromise
    ]);
    ...
    const levelData = normalizeLevelRows(rawLevelData || []);
...
function normalizeLevelRows(rows) {
  ...
  return rows.map(row => {
    if (row && typeof row.username !== 'string' && typeof row.studentid === 'string') {
      return { ...row, username: row.studentid };
    }
    return row;
  });
}
...
function computeTerm1Overview(username, theoryRows, levelRows) {
  ...
  const theory = theoryRows.filter(row => sameStudent(row, normalizedUsername));
  const levels = levelRows.filter(row => sameStudent(row, normalizedUsername));
...
function sameStudent(row, normalizedUsername) {
  if (!row) return false;
  ...
  try {
    return rawUsername.toLowerCase() === normalizedUsername;
  } catch (err) {
    return false;
  }
}
