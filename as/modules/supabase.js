...
export async function fetchProgressCounts() {
  ...
  try {
    ...
    const maxGrade = tData.length * LAYER_WEIGHTS[4];
    const normalizedGrade = maxGrade > 0 ? (rawGrade / maxGrade) * 100 : 0;
    const term1Grade = Math.min(100, Math.max(0, normalizedGrade));
